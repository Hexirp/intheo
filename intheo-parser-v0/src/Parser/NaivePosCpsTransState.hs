...
module Parser.NaivePosCpsTransState where
  ...
  newtype Parser p s m a = Parser { runParser :: forall r. s -> m r -> (a -> p -> s -> m r -> m r) -> m r }
  ...
  instance Monoid p => Alternative (Parser p s m) where
  ...
  instance Monoid p => MonadPlus (Parser p s m) where
