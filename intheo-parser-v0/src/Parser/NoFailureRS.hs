...
module Parser.NoFailureRS where
  import Prelude

  import Control.Applicative
  import Control.Monad

  class Monoid m => StrictMonoid m where
    rMonoid :: m -> ()
  ...
  instance (Monoid p, StrictMonoid p) => Applicative (Parser e p s m) where
    pure :: forall e p s m a. (Monoid p, StrictMonoid p) => a -> Parser e p s m a
    pure x = i0 x
     where
      i0 :: a -> Parser e p s m a
      i0 x = Parser (i1 x)
      i1 :: a -> (forall r. e -> s -> (a -> p -> s -> m r) -> m r)
      i1 x = \e s c -> let p' = mempty in rMonoid p' `seq` c x p' s

    (<*>) :: forall e p s m a b. (Monoid p, StrictMonoid p) => Parser e p s m (a -> b) -> Parser e p s m a -> Parser e p s m b
    x0 <*> x1 = i0 x0 x1
     where
      ...
      i3 x0v x0p x1v x1p x1s c = let p' = x0p <> x1p in rMonoid p' `seq` c (x0v x1v) p' x1s

  instance (Monoid p, StrictMonoid p) => Monad (Parser e p s m) where
    (>>=) :: forall e p s m a b. (Monoid p, StrictMonoid p) => Parser e p s m a -> (a -> Parser e p s m b) -> Parser e p s m b
    x0 >>= x1 = i0 x0 x1
     where
      ...
      i5 x0p x1v x1p x1s c = let p' = x0p <> x1p in rMonoid p' `seq` c x1v p' x1s

  instance (Monoid p, StrictMonoid p, Alternative m) => Alternative (Parser e p s m) where
    empty :: forall e p s m a. (Monoid p, StrictMonoid p, Alternative m) => Parser e p s m a
    empty = i0
     where
      i0 :: Parser e p s m a
      i0 = Parser i1
  ...
  instance (Monoid p, StrictMonoid p, Alternative m) => MonadPlus (Parser e p s m) where

  onHold :: forall e p s m a. (Monoid p, StrictMonoid p) => Parser e p s m a -> Parser e p s m (a, p, s)
  onHold x = i0 x
   where
    ...
    i2 xv xp xs s c = let p' = mempty in rMonoid p' `seq` c (xv, xp, xs) p' s
  ...
  refuse :: forall e p s m a. (Monoid p, StrictMonoid p) => (a, p, s) -> Parser e p s m ()
  refuse x = pure ()

  backtrack :: forall e p s m a b. (Monoid p, StrictMonoid p) => Parser e p s m (Either a b) -> (a -> Parser e p s m b) -> Parser e p s m b
  backtrack x0 x1 = do
    x0 <- onHold x0
    x2 <- case x0 of
